...
test(
  "prepareContext - Success: Load resources and tools correctly",
  async () => {
...
test("prepareContext - Failure: Context directory does not exist", async () => {
  ...
  await expect(prepareContext(nonExistentDir, options)).rejects.toThrow(
    "Context dir",
  );
}, DENO_PERMISSIONS);
...
test(
  "prepareContext - Success: Load with no resources and tools folders",
  async () => {
    const tempDir = makeTempDirSync();
...
test(
  "prepareContext - Failure: context.json invalid format (missing required fields)",
  async () => {
    const tempDir = makeTempDirSync();
