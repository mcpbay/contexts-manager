...
import { ITSExecuteOptions } from "../src/utils/ts-execute.util.ts";
...
Deno.test("MCPContext - Load multiple contexts and verify tool execution behavior", async () => {
  ...
  try {
    ...
    Deno.writeTextFileSync(join(context1Dir, "context.json"), JSON.stringify(context1Config));
    Deno.writeTextFileSync(join(context1Dir, "deno.json"), JSON.stringify({
      imports: { "helper": "./helper.ts" }
    }));
    Deno.writeTextFileSync(join(context1Dir, "helper.ts"), "export const val = 'from-context1';");
    
    const tools1Dir = join(context1Dir, "tools");
    Deno.mkdirSync(tools1Dir);
    Deno.writeTextFileSync(join(tools1Dir, "tool1.ts"), `
    ...
    Deno.writeTextFileSync(join(context2Dir, "context.json"), JSON.stringify(context2Config));
    Deno.writeTextFileSync(join(context2Dir, "deno.json"), JSON.stringify({
      imports: { "helper": "./helper.ts" }
    }));
    Deno.writeTextFileSync(join(context2Dir, "helper.ts"), "export const val = 'from-context2';");
    
    const tools2Dir = join(context2Dir, "tools");
    Deno.mkdirSync(tools2Dir);
    Deno.writeTextFileSync(join(tools2Dir, "tool2.ts"), `
      import { val } from "helper";
      export function toolMeta() {
        return {
          name: "tool2",
    ...
    Deno.writeTextFileSync(join(resources2Dir, "res2.ts"), `
    ...
    Deno.writeTextFileSync(join(resources1Dir, "res1.md"), `---
name: res1
description: Res 1
title: Title 1
---
Content 1`);
    ...
    Deno.writeTextFileSync(join(resources2Dir, "res3.md"), `---
name: res3
description: Res 3
title: Title 3
---
Content 3`);
    ...
    // Tool 3 in Context 1
    Deno.writeTextFileSync(join(tools1Dir, "tool3.ts"), `
      export function toolMeta() {
        return {
          name: "tool3",
          description: "Tool 3",
    ...
    const resp3 = await mcpContext.executeTool("tool3", { text: "hello" }, options);
...
Deno.test("MCPContext - Crash if TypeScript tool found but deno.json is missing", async () => {
  ...
  try {
    ...
    Deno.writeTextFileSync(join(toolsDir, "tool.ts"), "export function toolMeta() { return { name: 't', description: 'd', inputSchema: {} }; }");
    ...
    await assertRejects(() => mcpContext.loadContext(contextDir, options), Error, "requires a `deno.json` file");
...
Deno.test("MCPContext - Success if no TypeScript tools/resources and deno.json is missing", async () => {
  ...
  try {
    ...
    Deno.writeTextFileSync(join(resourcesDir, "res.md"), "---\\nname: res\\ndescription: d\\n---\\nMarkdown content");
