...
test("MCPContext - Load multiple contexts and verify tool execution behavior", async () => {
...
test("MCPContext - Crash if TypeScript tool found but deno.json is missing", async () => {
  ...
  try {
    ...
    await expect(mcpContext.loadContext(contextDir, options)).rejects.toThrow("requires a `deno.json` file");
  } finally {
    removeSync(tempDir);
  }
}, DENO_PERMISSIONS);
